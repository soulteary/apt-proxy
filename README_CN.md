...
预编译二进制（[releases 页面](https://github.com/soulteary/apt-proxy/releases) 上的 tar.gz 以及 `.deb` / `.rpm` / `.apk` 安装包）：

- Linux：`amd64`（x86_64）、`386`（i386）、`arm64`（ARMv8）、`arm`（ARMv6 与 ARMv7）
- macOS：`amd64`（Intel）与 `arm64`（Apple Silicon）

多架构 Docker 镜像（`soulteary/apt-proxy` 与 `ghcr.io/soulteary/apt-proxy`）：

- `linux/amd64`
- `linux/arm64`
- `linux/arm/v7`

> 说明：ARMv6 仅以独立二进制形式发布，未提供 ARMv6 的 Docker 镜像。
...
- `host_pattern` — 可选，用于匹配请求 `Host` 头的正则。适用于仓库直接放在域名根目录、路径里没有前缀可供 `url_pattern` 匹配的情况（例如 `deb http://security.debian.org <suite>-security main`，或 `apt.armbian.com`）。仅在 `url_pattern` 未命中时才会尝试；命中后会把整个请求路径拼到上游镜像后。请使用 `^...$` 锚定，避免相似域名冒充你的发行版。
